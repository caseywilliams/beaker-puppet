  ...
  module DSL
    module InstallUtils
      ...
      module FOSSUtils
        ...
        #
        # The default is to install puppetserver 2.x (which goes with puppet
        # 4.x and puppet-agent 1.x) from the 'puppet' officail repo.
        #
        # @param [Host] host An object implementing {Beaker::Hosts}'s interface
        # @option opts [String] :version Specific puppetserver version
        #     Defaults to the latest version. If set, overrides all other
        #     options and installs the specific version from Puppet's internal build servers.
        # @option opts [Boolean] :nightlies Whether to install from nightlies
        #     Defaults to false.
        # @option opts [String] :release_stream Which release stream to install repos from
        #     Defaults to 'puppet', which includes server 2.x and puppet 4.x.
        #     Other valid values are puppet5, puppet6, etc.
        # @option opts [String] :nightly_builds_url Nightly builds URL
        #     Defaults to DEFAULT_NIGHTLY_BUILDS_URL.
        # @option opts [String] :yum_nightly_buidls_url Specific nightly builds URL for yum
        #     Defaults to DEFAULT_NIGHTLY_BUILDS_URL.
        # @option opts [String] :apt_nightly_buidls_url Specific nightly builds URL for apt
        #     Defaults to DEFAULT_NIGHTLY_BUILDS_URL.
        # @option opts [String] :dev_builds_url Puppet internal builds URL
        ...
        def install_puppetserver_on(host, opts = {})
          ...
          if opts[:version] && opts[:version] != 'latest'
            ...
            install_from_build_data_url('puppetserver', build_yaml_uri)
            return
          end

          # Determine the release stream's name, for repo selection. The default
          # is puppet/PC1, which installs puppetserver 2.x, puppet 4.x, and
          # puppet-agent 1.x. Other valid values are 'puppet5' or 'puppet6'.
          ...
          if opts[:nightlies]
            ...
            if nightly_builds_url == DEFAULT_NIGHTLY_BUILDS_URL
              # Puppet's nightlies use special paths for these:
